...
// in simulator we redefine to format for Push_Offer Developer console
...
{\
	NSString * exceptionName = [@"org.push_offer." stringByAppendingString:NSStringFromClass([self class])];\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
\
+ (void) throwException:(NSString *) reason subreason:(NSString*)subreason location:(NSString *)location\
{\
	NSString * exceptionName = @"org.push_offer";\
	TiExceptionThrowWithNameAndReason(exceptionName,reason,subreason,location);\
}\
...
enum {
	UIPush_OfferNativeItemNone = -1, 
	UIPush_OfferNativeItemSpinner = -2,
	UIPush_OfferNativeItemProgressBar = -3,
	
	UIPush_OfferNativeItemSlider = -4,
	UIPush_OfferNativeItemSwitch = -5,
	UIPush_OfferNativeItemMultiButton = -6,
	UIPush_OfferNativeItemSegmented = -7,
	
	UIPush_OfferNativeItemTextView = -8,
	UIPush_OfferNativeItemTextField = -9,
	UIPush_OfferNativeItemSearchBar = -10,
	
	UIPush_OfferNativeItemPicker = -11,
	UIPush_OfferNativeItemDatePicker = -12,
	
	UIPush_OfferNativeItemInfoLight = -13,
	UIPush_OfferNativeItemInfoDark = -14,
	
	UIPush_OfferNativeItemDisclosure = -15,
	
	UIPush_OfferNativeItemContactAdd = -16
};
...
#endif
/**
 *	TiThreadPerformOnMainThread should replace all Push_Offer instances of
