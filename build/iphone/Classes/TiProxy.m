...
//Rare exceptions to indicate a bug in the _push_offer code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_push_offer = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_push_offer]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.push_offer.module"
